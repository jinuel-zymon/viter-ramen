...
const RamenTable = () => {
  ...
  const {
    ...
    queryKey: ["ramen", onSearch, store.isSearch],
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `/v2/ramen/search`, // search endpoint
        `/v2/ramen/page/${pageParam}`, // list endpoint
  ...
  return (
    ...
    {store.isConfirm && 
      <ModalConfirm
        queryKey="ramen"
        mysqlApiArchive ={`/v2/ramen/active/${id}`}
        active={isActive}
      />
    }
    {store.isDelete && 
      <ModalDelete
        mysqlApiDelete ={`/v2/ramen/${id}`}
        queryKey="ramen" 
      />
    }
